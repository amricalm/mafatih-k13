		...
		<div id="content" class="box">
			...
			<div id="tab01">
    			...
    			<table width="55%" class="tables" >
                    ...
                    if($siswa->num_rows() > 0)
                    {
                        foreach($siswa->result() as $row)
                        {
                            ...
                            echo '<td><a style="text_decoration:none;" href="javascript:ambil('."'".trim($row->nis)."'".','."'".trim($row->nama_lengkap)."'".','."'".trim($row->kelas)."'".')">'.$row->nis.'</a></td>';
                            echo '<td><a style="text_decoration:none;" href="javascript:ambil('."'".trim($row->nis)."'".','."'".trim($row->nama_lengkap)."'".','."'".trim($row->kelas)."'".')">'.$row->nama_lengkap.'</a></td>';
                            echo '</tr>';
                            $seq++;
                        }
                    }
                ...
                <script type="text/javascript">
                	function ambil(nis,nama_lengkap,kelas)
                	{
                		if(window.opener)
                		{
