<?php
class Eskulsiswa extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('eskulsiswa_model');
    ...
    function daftar()
    {
        ...
        $data['eskulsiswa']                     = $this->eskulsiswa_model->get();
        $data['pilihkelas']                     = '';
        ...
        $this->load->view('ekstrakurikuler/daftar_eskulsiswa',$data);
    }
    function get_eskul_nis()
    {
        ...
        $data['kd_eskul']                       = '';
        $sdata                                  = $this->eskulsiswa_model->get_kelas_nis($data);
        $tabel                                  = '';
        $i                                      = 1;
        $seqelement                             = 0;
        ...
        {   
            ...
            $tabel                              .= '<tr'.$bg.'>';
            $tabel                              .= '<td>'.$i.'</td>';
            $tabel                              .= '<td><input type="hidden" name="kd_eskul[]" id="kd_eskul" value="'.$rowdata->kd_eskul.'"/>'.$rowdata->nm_eskul.'</td>';
    ...
    function simpan()
    {
        ...
        {
           $data['kd_eskul']    = $kd_eskul[$i];
           $data['hasil']       = $this->input->post('hasil'.$i);
           if($this->eskulsiswa_model->get_kelas_nis($data)->num_rows() > 0)
           {
                $this->eskulsiswa_model->update($data);
           }
           else
           {
                $this->eskulsiswa_model->simpan($data);
           }
        }
        redirect('eskulsiswa/daftar/'.str_replace(' ','+',$data['kelas']));
